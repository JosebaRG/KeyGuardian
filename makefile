...
D-INC = ./inc
D-SRC = ./src
D-INC-TB = ./submodule/toolbox/inc
D-SRC-TB = ./submodule/toolbox/src
...
DEPS = $(wildcard $(D-INC)/*.h) $(wildcard $(D-INC-TB)/*.h)
SRC = $(wildcard $(D-SRC)/*.c) $(wildcard $(D-SRC-TB)/*.c)
...
# FOR SOURCE CODE FROM PROJECT
$(D-OBJ)/%.o: $(D-SRC)/%.c $(DEPS)
	@echo "Compiling: $< into $@"
	mkdir -p $(D-OBJ)
	$(COMPILER) -g$(DEBUG) -c -o $@ $< $(CFLAGS)

# FOR TOOLBOX SOURCE CODE
$(D-OBJ)/%.o: $(D-SRC-TB)/%.c $(DEPS)
	@echo "Compiling: $< into $@"
	mkdir -p $(D-OBJ)
	$(COMPILER) -g$(DEBUG) -c -o $@ $< $(CFLAGS)
